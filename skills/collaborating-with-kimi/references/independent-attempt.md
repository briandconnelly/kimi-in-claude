...
Use this pattern to obtain one Claude attempt and one Kimi attempt from the same neutral problem,
...
4. Produce and finalize Claude's attempt in full before any call that can return Kimi's answer.
...
5. Only after Claude's attempt is finalized, poll and fetch Kimi's result per the background-jobs
   reference.
...
order cannot be repaired by intent: once Kimi's answer is in context, everything drafted afterward
...
Kimi's answer entered context before Claude's attempt was finalized; reclassify the call as

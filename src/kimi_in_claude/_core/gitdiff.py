"""Gather a git diff for review. We run git ourselves so Kimi gets exactly the
...
#                           outside the Kimi sandbox (mirrors worktree.py's hardening).
...
def _global_excludes_flags(cwd: str, timeout: int) -> list[str]:
    ...
    — with ``untracked="include"`` — send its contents to Kimi.
...
def _untracked_new_file_diff(
    ...
    # consumer once ls-files is killed — Kimi #331 review, HIGH).
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        def _budget() -> float:
            ...
            # materially past `timeout` (Kimi #331 review, HIGH). Raises the moment it is spent
        ...
        def _index_untracked(records: Iterator[str]) -> int:
            ...
            for record in records:
                ...
                if len(path.encode("utf-8", "surrogateescape")) > _MAX_UNTRACKED_PATH_BYTES:
                    ...
                    # reader's truncation marker could (Kimi #331 review, MEDIUM).
                    raise RuntimeError(

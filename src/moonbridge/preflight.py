...
# re-probes after HELP_CACHE_TTL_SECONDS so an in-place `kimi` upgrade is noticed.
_cache: tuple[float, FlagSupport] | None = None


def _probe_help() -> str:
    """Return the `kimi --help` text, or "" on any failure (callers fail open)."""
    run = runtime.run_sync_capture(
        [cli_contract.KIMI_BIN, *cli_contract.EXEC_HELP_ARGS], timeout_seconds=10
    )
...
def flag_support(force: bool = False) -> FlagSupport:
    """Cached FlagSupport for the installed `kimi`. force=True bypasses the cache."""
...
def missing_expected_flags(fs: FlagSupport) -> list[str]:
    ...
    the probe could not run. Diagnostic only — surfaced by kimi_status, it does
    NOT gate execution."""

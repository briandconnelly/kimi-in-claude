...
Kimi's sandbox, so every invocation is prefixed with ``_hardening_flags`` (see
...
def _hardening_flags(repo: str, timeout: int, *, aliases: Iterable[str] = ()) -> list[str]:
    ...
    in Kimi's sandbox): repo hooks and fsmonitor (``_base_hardening_flags``) plus every
...
def _count_untracked(repo: str, timeout: int) -> int:
    ...
    ``kimi_delegate_dry_run`` and ``kimi_review_changes``/``kimi_dry_run`` can't drift
    (#323).
...
def sanitize_prose(text: str | None, aliases: Iterable[str]) -> str | None:
    ...
    - **Relativize, then redact** shortens a labelled worktree-prefixed value below the
      ...
      ``api_key=./abcdefgh`` and the secret escapes. Kimi sees the worktree path, so an

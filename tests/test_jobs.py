...
from kimi_in_claude._core.jobs import DiscardOutcome, JobStore
...
def test_start_status_result_done(tmp_path):
    ...
    cwd = str(tmp_path)
    job_id, started = store.start(_factory(_WRITE_DONE), cwd, kind="kimi_delegate")
...
def test_cancel_removes_declared_external_paths(tmp_path):
    ...
    cwd = str(tmp_path)
    job_id, _ = store.start(_declare_factory(str(root)), cwd, kind="kimi_delegate")
...
from kimi_in_claude._core import jobs  # noqa: E402
...
def test_start_cleans_external_paths_if_meta_write_fails(tmp_path, monkeypatch):
    ...
    with pytest.raises(OSError):
        store.start(_declare_factory(str(root)), cwd, kind="kimi_delegate")
...
def test_poll_backoff_grows_and_is_bounded():
    from kimi_in_claude._core.jobs import (
...
def test_status_running_poll_after_ms_grows(tmp_path):
    from kimi_in_claude._core.jobs import DEFAULT_POLL_AFTER_MS, MAX_POLL_AFTER_MS
...
def test_status_dict_includes_activity_fields(tmp_path: Path):
    ...
    jid, _ = store.start(lambda jd: ["true"], cwd=str(tmp_path), kind="kimi_consult")
...
def test_activity_observer_end_to_end_into_job_store(tmp_path: Path):
    ...
    from kimi_in_claude import _worker
    ...
    job_id, _ = store.start(lambda jd: ["true"], cwd=cwd, kind="kimi_delegate")
    jd = store._job_dir(cwd, job_id)
...
def test_idempotent_first_start_creates_and_indexes(tmp_path):
    ...
    out = store.start_idempotent(
        _factory(_SLEEP), cwd, kind="k", tool="kimi_consult", key="k1", arg_hash="AH"
    )
...
def test_idempotent_same_key_different_args_conflicts(tmp_path):
    ...
    b = store.start_idempotent(
        _factory(_SLEEP), cwd, kind="k", tool="kimi_consult", key="k1", arg_hash="OTHER"
    )
...
def test_idempotent_process_lock_contention_degrades_to_in_progress(tmp_path):
    ...
    from kimi_in_claude._core import jobs as jobs_mod
...
def test_result_ok_true_for_stored_success(tmp_path):
    store = _store(tmp_path)
    cwd = str(tmp_path)
    job_id, _ = store.start(_factory(_WRITE_DONE), cwd, kind="kimi_delegate")
    assert _wait_terminal(store, cwd, job_id) == "done"
...
def test_result_ok_false_for_stored_error(tmp_path):
    store = _store(tmp_path)
    cwd = str(tmp_path)
    job_id, _ = store.start(_factory(_WRITE_ERROR), cwd, kind="kimi_consult")
    assert _wait_terminal(store, cwd, job_id) == "done"

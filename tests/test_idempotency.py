...
def test_key_digest_is_unambiguous_across_tool_and_key():
    ...
    assert idem.key_digest("kimi_consult", "k1") == idem.key_digest("kimi_consult", "k1")
...
def test_first_reserve_wins_and_writes_reserved_record(tmp_path):
    idx = _idx(tmp_path)
    out = idx.reserve("kimi_consult", "k1", "AH1", _resolver())
...
def test_published_identical_call_replays(tmp_path):
    ...
    again = idx.reserve(
        "kimi_consult", "k1", "AH1", _resolver(**{"job-123": {"exists": True, "terminal": False}})
    )
...
def test_same_key_different_args_conflicts(tmp_path):
    ...
    _publish(idx, out, "job-123")
    clash = idx.reserve(
        "kimi_consult", "k1", "AH2", _resolver(**{"job-123": {"exists": True, "terminal": True}})
    )
...
def test_consumed_job_is_result_unavailable_within_window(tmp_path):
    ...
    gone = idx.reserve("kimi_consult", "k1", "AH1", _resolver())
    assert gone.kind == idem.UNAVAILABLE


def test_reserved_but_unpublished_is_in_progress(tmp_path):
    idx = _idx(tmp_path)
    idx.reserve("kimi_consult", "k1", "AH1", _resolver())  # won, never published
    second = idx.reserve("kimi_consult", "k1", "AH1", _resolver())
...
def test_empty_placeholder_reads_as_in_progress(tmp_path):
    ...
    (d / f"{idem.key_digest('kimi_consult', 'k1')}.json").write_text("")  # torn/mid-write
    out = idx.reserve("kimi_consult", "k1", "AH1", _resolver())
...
def test_corrupt_record_fails_closed_unavailable(tmp_path):
    ...
    d = tmp_path / "ws" / ".idem"
    d.mkdir(parents=True)
    (d / f"{idem.key_digest('kimi_consult', 'k1')}.json").write_text("{not json")
    out = idx.reserve("kimi_consult", "k1", "AH1", _resolver())
...
def test_undecodable_record_fails_closed_unavailable(tmp_path):
    ...
    d = tmp_path / "ws" / ".idem"
    d.mkdir(parents=True)
    (d / f"{idem.key_digest('kimi_consult', 'k1')}.json").write_bytes(b"\xff\xfe\xfa")
    out = idx.reserve("kimi_consult", "k1", "AH1", _resolver())
    assert out.kind == idem.UNAVAILABLE
...
def test_stale_reservation_past_horizon_is_swept_and_rewon(tmp_path):
    ...
    idx.reserve("kimi_consult", "k1", "AH1", _resolver())  # won, unpublished, stale
    time.sleep(0.01)
    out = idx.reserve("kimi_consult", "k1", "AH1", _resolver())
...
def test_sweep_removes_undecodable_record_past_horizon(tmp_path):
    ...
    d = tmp_path / "ws" / ".idem"
    d.mkdir(parents=True)
    path = d / f"{idem.key_digest('kimi_consult', 'k1')}.json"
...
def _write_raw(tmp_path, text):
    ...
    p = d / f"{idem.key_digest('kimi_consult', 'k1')}.json"
    p.write_text(text)
    return p
...
def test_unknown_future_version_fails_closed(tmp_path):
    ...
    out = idx.reserve("kimi_consult", "k1", "AH1", _resolver(j={"exists": True, "terminal": True}))
    assert out.kind == idem.UNAVAILABLE
...
def test_reserve_removes_placeholder_when_initial_write_fails(tmp_path, monkeypatch):
    ...
    with pytest.raises(OSError) as excinfo:
        idx.reserve("kimi_consult", "k1", "AH1", _resolver())
    assert excinfo.value is boom  # original error propagates unchanged
    assert not idx._path("kimi_consult", "k1").exists()  # placeholder rolled back
    monkeypatch.undo()
    out = idx.reserve("kimi_consult", "k1", "AH1", _resolver())
...
def test_publish_self_heals_vanished_reservation(tmp_path):
    ...
    replay = idx.reserve("kimi_consult", "k1", "AH1", live)
    assert replay.kind == idem.REPLAY and replay.job_id == "job-1"  # not a second win
    clash = idx.reserve("kimi_consult", "k1", "OTHER", live)
...
def test_transient_read_oserror_is_io_error_not_unavailable(tmp_path, monkeypatch):
    ...
    out = idx.reserve("kimi_consult", "k1", "AH1", live)
    assert out.kind == idem.WON
    _publish(idx, out, "job-1")
    ...
    out2 = idx.reserve("kimi_consult", "k1", "AH1", live)
    ...
    out3 = idx.reserve("kimi_consult", "k1", "AH1", live)

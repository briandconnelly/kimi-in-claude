...
def test_delegate_prompt_asks_for_repository_relative_paths():
    """The prompt-side half of #412: Kimi's working directory is a worktree that is deleted
...
def test_status_version_unsupported_warning(monkeypatch, clean_env):
    monkeypatch.setattr(server.kimi, "kimi_version", lambda: "kimi-cli 0.99.0")
...
async def test_review_commit_scope_label(monkeypatch, clean_env, tmp_path):
    ...
    async def fake(prompt, **k):
        seen["prompt"] = prompt
        return server.kimi.KimiRunResult(
            run=server.kimi.runtime.CommandRun("", "", 0, 1, False),
...
async def test_review_invalid_workspace(clean_env):
    res = await server.kimi_review_changes(scope="working_tree", workspace_root="relative/x")
    assert res["ok"] is False
